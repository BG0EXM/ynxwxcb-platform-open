  ...
  <div>
    <el-card shadow="never">
      ...
      <el-table :data="list" stripe v-loading="loading" empty-text="暂无大事记">
        ...
        <el-table-column prop="title" label="重大事项" min-width="180" show-overflow-tooltip />
        <el-table-column prop="content" label="详情" min-width="180" show-overflow-tooltip />
    ...
    <el-dialog v-model="dialogVisible" :title="editId ? '编辑大事记' : '录入大事记'" width="560px">
      <el-form :model="form" label-width="90px">
        ...
        <el-form-item label="重大事项" required>
          <el-input v-model="form.title" placeholder="如：圆满完成 XX 专项工作" />
        </el-form-item>
...
const editId = ref(0)
const form = ref({ department_id: '', period: dayjs().format('YYYY-MM-DD'), title: '', content: '' })
...
const openCreate = () => {
  editId.value = 0
  form.value = { department_id: authStore.isAdmin ? '' : (authStore.user?.department_id || ''), period: dayjs().format('YYYY-MM-DD'), title: '', content: '' }
...
const openEdit = (row) => {
  editId.value = row.id
  form.value = {
    department_id: row.department_id, period: row.period,
